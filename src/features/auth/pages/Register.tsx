  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)}
        className="w-full bg-surface-container-lowest border border-outline-variant rounded-xl py-3.5 pl-12 pr-10 text-left relative text-base focus:outline-none focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 transition-all"
      >
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center min-h-dvh">
          <div className="absolute inset-0 bg-black/50" onClick={close} aria-hidden />
          <div className="relative w-full lg:max-w-md bg-white rounded-t-2xl lg:rounded-2xl shadow-2xl flex flex-col max-h-[75vh] pb-[env(safe-area-inset-bottom,0px)]">
...
export default function Register({ onBack, onSuccess, onSignIn }: { onBack: () => void; onSuccess: (data: Record<string, unknown>) => void; onSignIn?: () => void }) {
  ...
  const handleStep2 = (e: React.FormEvent) => {
    ...
    for (const v of vehicles) {
      ...
      if (!isGeneratorType(v.type) && !isValidPlateFormat(v.plate)) { setError("Plate number may only contain letters, numbers, spaces, and hyphens (e.g. ABC-1234)."); return; }
