...
type RichQrPayload = {
  uid?: string;
  plate?: string;
  // Legacy fields — kept optional for backward-compatible decoding of older QR codes.
  firstName?: string;
  lastName?: string;
  barangay?: string;
  vehicleType?: string;
...
export function encodeQR(
  ...
  // QR encodes only the plate — station looks up everything else from Firestore.
  const trimmedPlate = plate?.trim().toUpperCase();
  if (trimmedPlate) {
    return `${QR_PREFIX}|${toBase64Url(JSON.stringify({ plate: trimmedPlate }))}`;
  }

  // Fallback: UID-only when no plate is available.
  if (uid) {
    return `${QR_UID_PREFIX}|${uid}`;
  }

  // Last resort: encode whatever we have so the QR is never empty.
  return `${QR_PREFIX}|${toBase64Url(JSON.stringify({ uid, plate: trimmedPlate }))}`;
}
...
export function decodeQR(encoded: string): DecodedQR | null {
  ...
  if (trimmed.startsWith(`${QR_PREFIX}|`)) {
    ...
    try {
      ...
      // Need at least uid or plate to identify the resident.
      if (!parsed.uid && !parsed.plate) return null;

      const firstCode = parsed.firstName ? namePart(parsed.firstName) : "";
      const lastCode = parsed.lastName ? namePart(parsed.lastName) : "";
      const date = parsed.registeredAt ? new Date(parsed.registeredAt) : new Date(0);
      const serial = parsed.registeredAt
        ? (date.getTime() - EXCEL_EPOCH) / 86400000
        : 0;
      ...
      return {
        ...
        vehicleType: parsed.vehicleType ? String(parsed.vehicleType).trim() : null,
